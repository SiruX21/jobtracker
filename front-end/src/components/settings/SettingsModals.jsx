...
function SettingsModals({
  ...
  return (
    <>
      ...
      {showEmailChangeModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-all duration-300 ease-in-out animate-fadeIn">
          <div className={`bg-white dark:bg-gray-800 rounded-xl p-6 w-full mx-4 transition-all duration-300 ease-in-out transform scale-100 shadow-2xl border border-gray-200 dark:border-gray-700 ${isMobile ? 'max-w-sm' : 'max-w-lg'}`}>
            ...
            {emailChangeStep === 'pending' && (
              <div className="text-center py-4">
                ...
                <h4 className={`font-medium text-gray-900 dark:text-white mb-2 ${isMobile ? 'text-base' : 'text-lg'}`}>
                  Email Change Request Sent
                </h4>
                <p className={`text-gray-700 dark:text-gray-300 mb-4 ${isMobile ? 'text-sm' : ''}`}>
                  We've sent confirmation links to both your current email ({user?.email}) and your new email ({newEmail}).
                </p>
                <p className={`text-gray-600 dark:text-gray-400 mb-4 ${isMobile ? 'text-xs' : 'text-sm'}`}>
                  Please check both inboxes and click the confirmation links to complete the email change process.
                </p>
                <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg border border-blue-200 dark:border-blue-800">
                  <p className={`text-blue-700 dark:text-blue-300 ${isMobile ? 'text-xs' : 'text-sm'}`}>
                    <strong>Note:</strong> You must confirm from both emails within 24 hours, or the request will expire.
                  </p>
                </div>
              </div>
            )}
